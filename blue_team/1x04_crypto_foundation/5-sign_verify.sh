#!/bin/bash
#
# 5-sign_verify.sh
# Signs or verifies a file using RSA + SHA-256.
#
# Usage:
#   Sign:   ./5-sign_verify.sh sign <file_path> <private_key_path>
#   Verify: ./5-sign_verify.sh verify <file_path> <signature_path> <public_key_path>

set -euo pipefail

usage() {
    echo "Usage:"
    echo "  Sign:   $0 sign <file_path> <private_key_path>"
    echo "  Verify: $0 verify <file_path> <signature_path> <public_key_path>"
    exit 1
}

if [[ $# -lt 1 ]]; then
    usage
fi

MODE="$1"

case "$MODE" in
    sign)
        if [[ $# -ne 3 ]]; then
            usage
        fi
        FILE_PATH="$2"
        PRIVATE_KEY="$3"

        if [[ ! -f "$FILE_PATH" ]]; then
            echo "Error: file '$FILE_PATH' not found."
            exit 1
        fi
        if [[ ! -f "$PRIVATE_KEY" ]]; then
            echo "Error: private key '$PRIVATE_KEY' not found."
            exit 1
        fi

        SIG_PATH="${FILE_PATH}.sig"
        openssl dgst -sha256 -sign "$PRIVATE_KEY" -out "$SIG_PATH" "$FILE_PATH"
        echo "Signed '$FILE_PATH' -> '$SIG_PATH'"
        ;;

    verify)
        if [[ $# -ne 4 ]]; then
            usage
        fi
        FILE_PATH="$2"
        SIG_PATH="$3"
        PUBLIC_KEY="$4"

        if [[ ! -f "$FILE_PATH" ]]; then
            echo "Error: file '$FILE_PATH' not found."
            exit 1
        fi
        if [[ ! -f "$SIG_PATH" ]]; then
            echo "Error: signature file '$SIG_PATH' not found."
            exit 1
        fi
        if [[ ! -f "$PUBLIC_KEY" ]]; then
            echo "Error: public key '$PUBLIC_KEY' not found."
            exit 1
        fi

        if openssl dgst -sha256 -verify "$PUBLIC_KEY" -signature "$SIG_PATH" "$FILE_PATH" 2>/dev/null; then
            exit 0
        else
            echo "Signature verification FAILED for '$FILE_PATH'"
            exit 1
        fi
        ;;

    *)
        usage
        ;;
esac
